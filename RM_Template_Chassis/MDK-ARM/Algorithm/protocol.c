...
{	
  ...
  uint8_t j;
	...
	addr = (uint8_t *)&SelfDefineFrame.custom_data.data_cmd_id;
	
	
	SelfDefineFrame.FrameHeader.DataLength = dwLength;//数据长度
		SelfDefineFrame.FrameHeader.Seq = 0;
	
	SelfDefineFrame.CmdID = cmdid; //命令  0x0301
  ...
  for(i = 0;i < dwLength;i++)
	{
  	...
  	i = sizeof(SelfDefineFrame.FrameHeader) + sizeof(SelfDefineFrame.CmdID)  + dwLength;//计算实际帧的长度
	  SelfDefineFrame.CRC16 = append_crc16_check_sum((uint8_t *)&SelfDefineFrame,i);

    HAL_UART_Transmit(&huart2,(uint8_t *)&SelfDefineFrame,sizeof(custom_dataFrame),100);
...
////  if(Robot.id<10)//红方
////  custom_data_t.cilent_id = red_Infantry_4;//客户端id
////  else //蓝方
//  custom_data_t.cilent_id = 0x0103;//客户端id
//  
//	custom_data_t.data1 = 100;
//	custom_data_t.data2 = 0;
//	custom_data_t.data3 = 2.0f;
//  custom_data_t.masks = (1<<5) || (0<<4) || (0<<3) || (1<<2) || (1<<1) || (1);
//	Send_FrameData(custom_data,(uint8_t *)&custom_data_t,sizeof(client_custom_data_t));
	
	
	/*2020年发送程序*/
		  custom_data_t.data_cmd_id = 0x0101;//数据内容id（固定）
  custom_data_t.sender_ID = Robot.id;//机器人id
//  if(Robot.id<10)//红方
//  custom_data_t.cilent_id = red_Infantry_4;//客户端id
//  else //蓝方
  custom_data_t.receiver_ID = 0x0103;//客户端id
  custom_data_t.graphic_name[0]=1;
	custom_data_t.graphic_name[1]=0;
	custom_data_t.graphic_name[2]=0;
	custom_data_t.operate_tpye=1;
	custom_data_t.graphic_tpye=0;
	custom_data_t.color=0;
	custom_data_t.start_x=100;
	custom_data_t.start_y=100;
	custom_data_t.end_x=1000;
	custom_data_t.end_y=1000;
	custom_data_t.width=10;
	custom_data_t.radius=0;
	custom_data_t.start_angle=0;
	custom_data_t.end_angle=0;
